...
static void     ArrayIterator_setval(ArrayIteratorClass *this, ...)
{
    ...
    this->_array->_tab[this->_idx] = va_new(this->_array->_type, &l);
    va_end(l);
}
...
static void     Array_ctor(ArrayClass *this, va_list *args)
{
    ...
    for (size_t i = 0; i < this->_size; i++) {
        va_copy(t, *args);
        this->_tab[i] = va_new(this->_type, &t);
        va_end(t);
    }
}
...
static void     Array_setitem(ArrayClass *this, ...)
{
    ...
    if (!this)
        raise("Bad argument given");
    va_start(l, this);
    index = va_arg(l, size_t);
    delete(this->_tab[index]);
    this->_tab[index] = va_new(this->_type, &l);
    va_end(l);
}
